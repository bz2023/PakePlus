...
window.addEventListener("resize", function () {
  if (hasSearchParams()) {
      ...
      // let h = parseFloat(urlParams.get('h')) || 800;
      // let w = parseFloat(urlParams.get('w')) || 600;
      // ggbApplet2.setSize(window.innerWidth / zoomx, (window.innerHeight) / zoomx);
      // ggbApplet2.setSize((h/zoomx), (w/zoomx));
...
function myonload() {
  ...
  if (hasSearchParams()) {
      ...
      let zoomx = parseFloat(urlParams.get('zoom')) || sc;
      let page = parseFloat(urlParams.get('page')) || 1;
      let h = parseFloat(urlParams.get('h')) || 600;
      let w = parseFloat(urlParams.get('w')) || 400;
      // let strsx = JSON.parse(localStorage.getItem('strs'));
      var parameters2 = {
          "id": "ggbApplet2", "scale": 1,  "showAlgebraInput": false, "showToolBar": false, "borderColor": "white", "language": "en", "enableRightClick": true, "enableShiftDragZoom": true, "enableLabelDrags": true,
      ...
      parameters2["width"] = (h/zoomx);
      parameters2["height"] = (w/zoomx);
      // parameters2["width"] = (h);
      // parameters2["height"] = (w);
...
function getPos(e) {
  ...
  const touch = e.touches ? e.touches[0] : e; // For touch events, get the first touch point
  return { x: touch.clientX - r.left, y: touch.clientY - r.top };
}

function startDrawing(e) {
  e.preventDefault(); // Prevents default behavior for touch devices
...
function draw(e) {
  ...
  if (selectionRect) {
    // 更新框选区域
    const touch = e.touches ? e.touches[0] : e;
    const curX = touch.clientX, curY = touch.clientY;
...
document.addEventListener('keydown', function(event) {
  if ((event.ctrlKey && event.shiftKey && event.key.toLowerCase() === 'o')  || (event.ctrlKey && event.key === 's')) {
    event.preventDefault();
    savefile(); 
  }
});
