...
async function processFiles() {
  ...
  window.file_name_list = file_name_list;
  window.base64_list = base64_list;

  let tmmk = false;
  let mycc = c;
  saveGgbBase();
  let tmmp = ggbApplet.getBase64();
  for (let i = 0; i < base64_list.length; i++) {
    AddPage();
  }
  for (let i = 1; i <= base64_list.length; i++) {
    labels_[mycc + i] = file_name_list[i-1]; 
    mylist_[mycc + i] = base64_list[i-1]; 
  }
  SetPage2(mycc);
  SetPage2(mycc+ base64_list.length);
  ggbApplet.setBase64(base64_list[base64_list.length-1]);
  labels_[c] = file_name_list[file_name_list.length-1]; 
  mylist_[mycc] = tmmp;
